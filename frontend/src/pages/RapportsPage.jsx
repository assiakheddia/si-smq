...
const REPORT_ELEMENTS = [
  ...
  { id: "charts", label: "Graphiques et visualisations", icon: "📉", default: false },
  { id: "reports_list", label: "Liste des rapports", icon: "📋", default: true },
  { id: "audit_details", label: "Détails d'audit", icon: "🔍", default: false },
  { id: "non_conformities", label: "Non-conformités", icon: "⚠️", default: false },
...
export default function RapportsPage() {
  ...
  const handleGenerateReport = () => {
    if (!reportName.trim()) {
      setExportToast({ message: "Veuillez donner un nom au rapport", type: "error" });
      setTimeout(() => setExportToast(null), 2500);
      return;
    }
    const selectedCount = selectedElements.length;
    setExportToast({ message: `Rapport "${reportName}" généré avec ${selectedCount} éléments au format ${reportFormat}`, type: "success" });
  ...
  const handleExport = (fmt, titre) => {
    setExportToast({ message: `Export ${fmt} de «${titre}» en cours…`, type: "success" });
    setTimeout(() => setExportToast(null), 2500);
  };

  // Stats for the report preview
  const totalReports = REPORTS.length;
  const finalizedReports = REPORTS.filter(r => r.statut === "Finalisé").length;
  const inProgressReports = REPORTS.filter(r => r.statut === "En révision").length;
  ...
  return (
    <>
      ...
      {showReportBuilder && (
        <div className="rap-builder-overlay" onClick={() => setShowReportBuilder(false)}>
          <div className="rap-builder-modal" onClick={(e) => e.stopPropagation()}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 24 }}>
              <div>
                <div style={{ fontFamily: "'Outfit',sans-serif", fontWeight: 800, fontSize: 22, color: "#111" }}>
                  🛠️ Créer un rapport personnalisé
                </div>
                <div style={{ fontSize: 13, color: C.muted, marginTop: 4 }}>
                  Sélectionnez les éléments que vous souhaitez inclure dans votre rapport
                </div>
              </div>
              <button 
            ...
            <div style={{ marginBottom: 20 }}>
              <label style={{ fontSize: 12, fontWeight: 700, color: C.muted, textTransform: "uppercase", letterSpacing: 0.5, display: "block", marginBottom: 6 }}>
                Nom du rapport
              </label>
              <input
                value={reportName}
            ...
            <div style={{ marginBottom: 20 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                ...
                <button
                  onClick={() => setSelectedElements(REPORT_ELEMENTS.map(el => el.id))}
              ...
              <div className="rap-element-grid">
                {REPORT_ELEMENTS.map((el) => (
                  ...
                  >
                    ...
                    <span className="check" style={{ color: C.primary }}>✓</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div style={{ display: "flex", gap: 12, marginBottom: 24 }}>
              <label style={{ fontSize: 12, fontWeight: 700, color: C.muted, textTransform: "uppercase", letterSpacing: 0.5, display: "flex", alignItems: "center", gap: 8 }}>
                Format
              </label>
      ...
      <div className="rap-wrap">
        ...
        <div style={{ 
          ...
          <div>
            <div style={{ fontFamily: "'Outfit',sans-serif", fontWeight: 900, fontSize: "clamp(20px,2.5vw,28px)", color: "#111" }}>
              Rapports
            </div>
        ...
        <div style={{ 
          ...
          {[
            { icon: "📋", label: "Total rapports", value: totalReports, color: "#1e3d2f" },
            { icon: "✅", label: "Finalisés", value: finalizedReports, color: "#22c55e" },
            { icon: "🔄", label: "En révision", value: inProgressReports, color: "#f59e0b" },
            { icon: "✏️", label: "Brouillons", value: draftReports, color: "#6b7280" },
            ...
            <div key={label} style={{ 
              ...
              <div>
                <div style={{ fontFamily: "'Outfit',sans-serif", fontWeight: 900, fontSize: 26, color: "#111", lineHeight: 1 }}>
                  {value}
                </div>
        ...
        <div className="rap-card" style={{ animationDelay: "0.1s" }}>
          ...
          <div style={{ 
            ...
            <div style={{ 
              ...
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#c4c8d1" strokeWidth="2" strokeLinecap="round">
            ...
            <span style={{ fontSize: 11, color: "#9ca3af", marginLeft: "auto" }}>
              <strong style={{ color: "#374151" }}>{filtered.length}</strong> rapport{filtered.length > 1 ? "s" : ""}
            </span>
          </div>
          ...
          {filtered.length === 0 ? (
            <div style={{ padding: "60px 20px", textAlign: "center", color: "#9ca3af" }}>
              ...
              <div style={{ fontWeight: 600, fontSize: 16, color: "#6b7280", marginBottom: 4 }}>
            ...
            filtered.map((r, i) => {
              ...
              return (
                <div 
                  ...
                  style={{ 
                    padding: "14px 22px", 
                    borderBottom: i < filtered.length - 1 ? "1px solid #f0f2f4" : "none",
                    display: "flex", 
                    alignItems: "center", 
                    gap: 14, 
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                      ...
                      <span style={{ fontSize: 11, color: "#9ca3af" }}>
                        par <span style={{ color: "#6b7280", fontWeight: 500 }}>{r.auteur}</span>
                      </span>
                  ...
                  <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
                    <button 
                      onClick={(e) => { e.stopPropagation(); handleExport("PDF", r.titre); }} 
                      style={{ 
                        padding: "4px 10px", 
                        borderRadius: 7, 
                    ...
                    <button 
                      onClick={(e) => { e.stopPropagation(); handleExport("Excel", r.titre); }} 
                      style={{ 
                        padding: "4px 10px", 
                        borderRadius: 7, 
                        border: "none", 
                      ...
                      onMouseEnter={(e) => { e.currentTarget.style.background = "#1e6b42"; e.currentTarget.style.transform = "scale(1.02)" }}
