...
/* ── statut backend (diagnostic) → frontend ── */
...
/* ── endpoint pour avancer le statut (transitions supportées par le backend) ── */
    ...
    >
      ...
      >
        ...
        <div style={{ padding: "20px 24px", minHeight: 260 }}>
          {errors._api && (
            <div style={{ background: "#fee2e2", border: "1px solid #fca5a5", borderRadius: 8, padding: "8px 12px", fontSize: 12, color: "#991b1b", marginBottom: 12 }}>
              {errors._api}
            </div>
          )}
          {step === 0 && (
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              <div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    const p = processusList.find((p) => p.id === parseInt(e.target.value));
                    set("processus_id", e.target.value);
                    set("processus_nom", p?.nom || "");
                  }}
                  style={{ ...inputStyle(errors.processus_id), appearance: "none" }}
                >
              ...
              <div>
                <label style={labelStyle}>Commentaire / Objectif</label>
                <textarea
          ...
          {step === 1 && (
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              <div>
                ...
                <select
                  value={form.auditeur_id}
                  onChange={(e) => {
                    const u = usersList.find((u) => u.id === parseInt(e.target.value));
                    set("auditeur_id", e.target.value);
                    set("auditeur_nom", u ? `${u.prenom} ${u.nom}` : "");
                  }}
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              <div style={{ background: "#f0fdf4", border: "1px solid #dcfce7", borderRadius: 10, padding: "14px 16px" }}>
                ...
                {[
                  ...
                  { label: "Date planif.", value: form.datePlan ? fmtDate(form.datePlan) : "—" },
...
export default function AuditsPage() {
  ...
  const handleStatusChange = async (auditId, newStatut) => {
    ...
    setAudits((prev) =>
      prev.map((a) => (a.id === auditId ? { ...a, statut: newStatut } : a)),
    );
  ...
  return (
    ...
    >
      ...
      {!loading && tab === 1 && <TabProgrammeCalendrier audits={audits} />}
      {!loading && tab === 2 && (
        <TabAudits
          audits={audits}
          ncs={ncs}
