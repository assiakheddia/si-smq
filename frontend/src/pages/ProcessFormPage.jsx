...
const FRENCH_WORDS = new Set([
  ...
  'opportunité', 'opportunités', 'bénéfice', 'bénéfices', 'impact',
]);
...
function RaciMatrix({ raci, onChange }) {
  ...
  return (
    <div>
              ...
              {raci.roles.map((role, ri) => (
                <th key={ri} style={styles.raciTh}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 4 }}>
                    ...
                    />
                    <button onClick={() => removeRole(ri)} style={styles.raciRemoveBtn}>×</button>
                  </div>
                </th>
              ))}
              <th style={{ ...styles.raciTh, background: "rgba(45,96,79,0.5)", cursor: "pointer" }} onClick={addRole}>
                + Rôle
              </th>
            </tr>
          </thead>
              ...
              <tr key={ai}>
                <td style={styles.raciActivityCell}>
                  <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    ...
                    />
                    <button onClick={() => removeActivity(ai)} style={{ ...styles.dynRemove, fontSize: 12, color: C.muted }}>✕</button>
                  </div>
                </td>
                {raci.roles.map((_, ri) => {
                  ...
                  return (
                    <td
                      key={ri}
                      style={{ ...styles.raciCell, background: s.background, color: s.color, border: `1px solid ${s.borderColor}` }}
                ...
                <td style={{ ...styles.raciActivityCell, background: "transparent" }} />
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <button type="button" style={{ ...styles.raciAddBtn, marginTop: 10 }} onClick={addActivity}>
        ＋ Ajouter une activité
      </button>

      <div style={{ marginTop: 14, padding: "12px 16px", background: C.lightBg, borderRadius: 10, fontSize: 12, color: C.muted }}>
        <strong>Légende :</strong>&nbsp;
        {[["R", "Responsable – exécute"], ["A", "Approbateur – valide"], ["C", "Consulté – donne avis"], ["I", "Informé – reçoit info"]].map(([k, v]) => (
          <span key={k} style={{ marginRight: 14 }}>
            <span style={{ ...RACI_STYLE[k], padding: "1px 7px", borderRadius: 4, fontWeight: 700, fontSize: 11, border: `1px solid ${RACI_STYLE[k].borderColor}` }}>{k}</span>
            &nbsp;{v}
          </span>
        ))}
      </div>
  ...
  return (
    <div style={styles.auditOverlay}>
      <div style={styles.auditBox}>
        ...
        <div style={{ fontSize: 18, fontWeight: 700, color: C.text, marginBottom: 6, textAlign: "center" }}>Envoyer à un auditeur externe</div>
        <div style={{ fontSize: 13, color: C.muted, marginBottom: 20, textAlign: "center", lineHeight: 1.6 }}>
          L'auditeur recevra la fiche processus complète ainsi que les dysfonctionnements identifiés pour confirmation et recommandations.
        </div>
  ...
  return (
    <div style={styles.diagOverlay}>
      <div style={styles.diagBox}>
        ...
        <div style={{ fontSize: 20, fontWeight: 700, color: C.text, marginBottom: 8 }}>Analyse en cours…</div>
        <div style={{ fontSize: 13, color: C.muted, marginBottom: 28, lineHeight: 1.7 }}>
          Notre moteur SMQ analyse votre fiche processus par rapport aux exigences ISO 9001.<br />
          Vérification des KPIs, risques, documentation, déroulement et RACI.
        </div>
        <div style={{ display: "flex", gap: 6, justifyContent: "center" }}>
          {[0, 1, 2].map((i) => (
            <div key={i} style={{ width: 10, height: 10, borderRadius: "50%", background: C.primary, animation: `pulse 1.2s ease-in-out ${i * 0.2}s infinite` }} />
          ))}
        </div>
      </div>
    </div>
...
function generateDysfonctionnements(form) {
  const dysf = [];
  const soon = (days) => new Date(Date.now() + days * 86400000).toISOString().split("T")[0];

  if (form.kpis.length === 0) {
    dysf.push({
      titre: "Absence d'indicateurs de performance (KPIs)",
      description: "Aucun KPI n'est défini pour ce processus, rendant le suivi des performances impossible.",
      consequences: "Impossibilité de mesurer l'efficacité du processus et de détecter les dérives qualité.",
      causes: "Manque de définition des objectifs mesurables lors de la conception du processus.",
      gravite: "Majeur",
      ameliorations: "Définir au minimum 2–3 KPIs pertinents avec valeurs cibles et seuils d'alerte documentés.",
  ...
  const highRisks = form.risques.filter((r) => parseInt(r.probabilite) * parseInt(r.gravite) >= 6);
  if (highRisks.length > 0) {
    dysf.push({
      ...
      consequences: "Exposition à des défaillances majeures pouvant affecter la qualité du service et la certification ISO 9001.",
      causes: "Plans de traitement des risques incomplets ou non formalisés dans la fiche processus.",
      gravite: "Critique",
      ameliorations: "Mettre en place des actions préventives documentées pour chaque risque critique avec responsable et échéance.",
      responsable: form.pilote || "Pilote",
      echeance: soon(14),
      statut: "Ouvert",
    });
  ...
  if (form.documents.length === 0) {
    dysf.push({
      ...
      consequences: "Manque de traçabilité documentaire, non-conformité potentielle lors des audits ISO 9001 (clause 7.5).",
      causes: "Processus non encore documenté ou documentation non référencée dans le système qualité.",
      gravite: "Majeur",
      ameliorations: "Identifier et référencer tous les documents applicables : procédures, modes opératoires, enregistrements.",
      responsable: form.pilote || "Pilote",
      echeance: soon(45),
      statut: "Ouvert",
    });
  ...
  if (form.etapes.length === 0) {
    dysf.push({
      ...
      description: "Les étapes chronologiques du processus ne sont pas documentées.",
      consequences: "Risque d'exécution non homogène selon les intervenants, difficultés de formation et d'audit.",
      causes: "Processus décrit de manière informelle sans formalisation des étapes et acteurs responsables.",
      gravite: "Majeur",
      ameliorations: "Décrire les étapes principales avec acteurs, entrées, sorties et durées. Compléter avec un diagramme BPMN.",
      responsable: form.pilote || "Pilote",
      echeance: soon(21),
      statut: "Ouvert",
    });
  ...
  if (!hasCells) {
    dysf.push({
      ...
      description: "Aucune responsabilité n'est attribuée dans la matrice RACI du processus.",
      consequences: "Ambiguïté sur les rôles et responsabilités pouvant mener à des doublons ou des lacunes dans l'exécution.",
      causes: "Matrice RACI non complétée lors de la création de la fiche processus.",
      gravite: "Mineur",
      ameliorations: "Compléter la matrice RACI en attribuant R, A, C, I pour chaque activité et chaque acteur concerné.",
      responsable: form.pilote || "Pilote",
      echeance: soon(7),
      statut: "Ouvert",
    });
  ...
  if (dysf.length === 0) {
    dysf.push({
      ...
      description: "L'analyse automatique n'a pas détecté de dysfonctionnements majeurs. Le processus est bien documenté.",
      consequences: "Aucune conséquence critique identifiée à ce stade.",
      causes: "N/A",
      gravite: "Mineur",
      ameliorations: "Maintenir le suivi des KPIs définis et procéder à des révisions périodiques de la fiche.",
      responsable: form.pilote || "Pilote",
      echeance: soon(90),
      statut: "En cours",
    });
...
export default function ProcessFormPage() {
  ...
  const validateField = useCallback((key, value) => {
    ...
    const textFields = ["designation", "objectif", "pilote", "sousDept", "objectifStrategique", "enjeuxStrategiques", "periode", "clients", "effectifs"];
  ...
  const validateAll = () => {
    const e = {};
    REQUIRED_FIELDS.forEach((f) => { if (!isNotEmpty(form[f])) e[f] = "Ce champ est obligatoire"; });
    if (form.email && !isValidEmail(form.email)) e.email = "Veuillez entrer une adresse email valide.";
    if (form.pilote && !isValidName(form.pilote)) e.pilote = "Le nom ne doit pas contenir de chiffres.";
    setErrors(e);
  ...
  const proceedPublish = () => {
    ...
    setPublished(true);
    setDirty(false);
    pushNotification("publish", "Processus publié", `Le processus «${form.designation}» a été publié avec succès.`);
  ...
  const handleDiagnostic = () => {
    ...
    setTimeout(() => {
      ...
      setDiagDone(true);
      pushNotification("diagnostic", "Diagnostic SMQ complété", `L'analyse du processus «${form.designation}» a identifié ${results.length} dysfonctionnement(s).`);
      setUnreadCount((n) => n + 1);
      showToast(`Analyse terminée — ${results.length} dysfonctionnement(s) identifié(s) ✓`);
    }, 2800);
  };
  ...
  const handleSendToAuditor = (email, message) => {
    ...
    pushNotification("audit", "Fiche envoyée à l'auditeur", `Le processus «${form.designation}» a été transmis à ${email} pour audit externe.`);
    setUnreadCount((n) => n + 1);
...
═══════════════════════════════════════════════════════════════ */
  ...
  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
    <SectionHeader num="1c" title="Opportunités d'amélioration" sub="Identifiez les axes d'amélioration pour ce processus" />
    <button
      type="button"
      style={styles.addBtn}
      onClick={() => {
  ...
  {opps.length === 0 ? (
    <div style={styles.historyPlaceholder}>
      ...
      <div style={{ fontWeight: 600, marginBottom: 4 }}>Aucune opportunité identifiée</div>
      <div style={{ fontSize: 12, opacity: 0.7 }}>Cliquez sur "Ajouter une opportunité" pour commencer.</div>
    </div>
  ) : (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      {opps.map((opp, i) => {
        const impactStyle = impactStyles[opp.impact] || impactStyles["Moyen"];
        const statutStyle = statutStyles[opp.statut] || statutStyles["Planifié"];
        const sourceStyle = sourceStyles[opp.source] || sourceStyles["Préparateur"];
          ...
          <div key={i} style={{
            ...
            <div style={{ 
              ...
              <button
                ...
                onClick={() => set("opportunites", opps.filter((_, idx) => idx !== i))}
              >
                ✕ Supprimer
              </button>
            </div>
            ...
            <div style={{ 
              ...
              <span style={{ fontSize: 11, color: C.muted, fontWeight: 500 }}>Créé par:</span>
              <span style={{
                fontSize: 12,
                fontWeight: 600,
                color: C.primary,
              ...
              <span style={{
                ...
                {opp.dateCreation ? new Date(opp.dateCreation).toLocaleDateString("fr-FR") : new Date().toLocaleDateString("fr-FR")}
              </span>
            </div>
          </div>
        );
  ...
  <div style={{ 
    ...
    <span style={{ 
      ...
      color: C.muted,
      fontStyle: "italic",
    }}>(Cliquez sur le statut pour le changer)</span>
  </div>
</div>
    );
  };
      ...
      <div style={styles.page}>
        ...
        <div style={styles.main}>
          ...
          <div style={styles.actionBar}>
            <button type="button" style={styles.btnCancel} onClick={handleCancel}>← Annuler</button>
            <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
              ...
              <button type="button" style={styles.btnDraft} onClick={handleDraft}>💾 Enregistrer brouillon</button>
              {published && diagDone && !auditSent && (
                <button type="button" style={{ ...styles.btnDraft, borderColor: "#3b82f6", color: "#1e40af" }} onClick={() => setShowAuditModal(true)}>
                  📧 Envoyer à l'auditeur
                </button>
              )}
              {!published && (
                <button type="button" style={styles.btnPublish} onClick={handlePublish}>🚀 Publier le processus</button>
              )}
              {published && !diagDone && (
                <button type="button" style={{ ...styles.btnPublish, background: `linear-gradient(135deg, ${C.primary}, #3a7a62)` }} onClick={() => setActiveTab(4)}>
                  🔬 Aller au diagnostic
                </button>
              )}
            </div>
