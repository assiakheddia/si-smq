...
export default function ParametresPage() {
  ...
  const [profile, setProfile] = useState({ nom: "Atir", prenom: "Zineb", email: "atir.zineb@esi.dz", role: "Admin Système", dept: "Direction Qualité", telephone: "+213 555 012 345" });
  ...
  const SECTIONS = [
    { id: "profil", label: "Profil utilisateur", icon: "👤" },
  ...
  const renderSection = () => {
    switch (activeSection) {
      case "profil":
        return (
          <Section icon="👤" title="Profil utilisateur">
            {/* Avatar */}
            <div style={{ display: "flex", alignItems: "center", gap: 20, marginBottom: 28, padding: "18px", background: C.lightBg, borderRadius: 14 }}>
              <div style={{ width: 72, height: 72, borderRadius: 18, background: "linear-gradient(135deg, #5ecf7a, #2d9e5f)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Outfit',sans-serif", fontWeight: 900, fontSize: 26, color: "#152b21", flexShrink: 0 }}>AZ</div>
              <div>
                <div style={{ fontFamily: "'Outfit',sans-serif", fontWeight: 800, fontSize: 20, color: C.text }}>{profile.prenom} {profile.nom}</div>
                <div style={{ fontSize: 13, color: C.muted, marginTop: 2 }}>{profile.role} · {profile.dept}</div>
                <button style={{ marginTop: 8, padding: "5px 14px", borderRadius: 8, border: `1.5px solid ${C.border}`, background: "transparent", color: C.primary, fontSize: 12, fontWeight: 600, cursor: "pointer" }}>
                  Changer l'avatar
                </button>
              </div>
            </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px 24px" }}>
              {[
                ...
                { key: "email", label: "Email", type: "email" }, { key: "telephone", label: "Téléphone", type: "tel" },
                { key: "role", label: "Rôle" }, { key: "dept", label: "Département" },
              ].map(({ key, label, type }) => (
                <div key={key}>
                  ...
                  <input
                    ...
                    style={{ width: "100%", padding: "10px 14px", borderRadius: 10, border: `1.5px solid ${C.border}`, background: "#f0ffeb", fontSize: 14, outline: "none", boxSizing: "border-box", fontFamily: "'Plus Jakarta Sans',sans-serif", color: C.text }}
      ...
      case "notifs":
        return (
          <Section icon="🔔" title="Préférences de notification">
            ...
            {[
              { key: "publication", label: "Publicationd de processus", sub: "Notif quand un processus est publié" },
      ...
      case "systeme":
        return (
          <Section icon="⚙️" title="Paramètres du système">
            <FieldRow label="Langue d'interface" sub="Langue utilisée dans l'application">
              <div style={{ position: "relative" }}>
                <select value={sysSettings.langue} onChange={(e) => setS("langue", e.target.value)} style={{ width: "100%", padding: "10px 14px", borderRadius: 10, border: `1.5px solid ${C.border}`, background: "#f0ffeb", fontSize: 14, outline: "none", appearance: "none", fontFamily: "'Plus Jakarta Sans',sans-serif", color: C.text }}>
            ...
            <FieldRow label="Format de date" sub="Format d'affichage des dates">
              <div style={{ position: "relative" }}>
                <select value={sysSettings.dateFormat} onChange={(e) => setS("dateFormat", e.target.value)} style={{ width: "100%", padding: "10px 14px", borderRadius: 10, border: `1.5px solid ${C.border}`, background: "#f0ffeb", fontSize: 14, outline: "none", appearance: "none", fontFamily: "'Plus Jakarta Sans',sans-serif", color: C.text }}>
            ...
            <div style={{ background: "#fff3cd", border: "1px solid #ffc107", borderRadius: 12, padding: "12px 16px", fontSize: 13, color: "#856404", marginTop: 8 }}>
      ...
      case "securite":
        return (
          <Section icon="🔒" title="Sécurité & accès">
            <div style={{ background: C.lightBg, borderRadius: 14, padding: "18px 20px", marginBottom: 24 }}>
              ...
              <div style={{ fontSize: 13, color: C.muted }}>Connecté depuis : Chrome · Windows 11 · ESI Alger</div>
              <div style={{ fontSize: 12, color: "#9ca3af", marginTop: 4 }}>Dernière connexion : {new Date().toLocaleDateString("fr-FR")} à {new Date().toLocaleTimeString("fr-FR", { hour: "2-digit", minute: "2-digit" })}</div>
            </div>
              ...
              {[
                ...
                <div key={key} style={{ marginBottom: 14 }}>
                  ...
                  <input type="password" value={passwords[key]} onChange={(e) => setPasswords((p) => ({ ...p, [key]: e.target.value }))} placeholder={placeholder}
                    style={{ width: "100%", padding: "10px 14px", borderRadius: 10, border: `1.5px solid ${C.border}`, background: "#f0ffeb", fontSize: 14, outline: "none", boxSizing: "border-box", fontFamily: "'Plus Jakarta Sans',sans-serif" }}
                    onFocus={(e) => e.target.style.borderColor = C.primary}
                    onBlur={(e) => e.target.style.borderColor = C.border}
                  />
                </div>
              ))}
              <button style={{ padding: "10px 24px", borderRadius: 10, border: "none", background: C.primary, color: "#fff", fontWeight: 700, fontSize: 14, cursor: "pointer", marginTop: 4 }}>
            ...
            <div style={{ paddingTop: 20, borderTop: `1px solid ${C.border}` }}>
              ...
              <button onClick={() => navigate("/login")} style={{ padding: "10px 24px", borderRadius: 10, border: "1.5px solid #fca5a5", background: "#fee2e2", color: "#991b1b", fontWeight: 700, fontSize: 14, cursor: "pointer" }}>
                Se déconnecter
              </button>
            </div>
          </Section>
      ...
      case "apropos":
        return (
          <Section icon="ℹ️" title="À propos de VERIDIA">
  ...
  return (
    <>
      ...
      {saved && (
        <div style={{ position: "fixed", bottom: 28, right: 28, background: C.primary, color: "#fff", borderRadius: 12, padding: "14px 22px", fontSize: 14, fontWeight: 600, zIndex: 9999, boxShadow: "0 8px 32px rgba(0,0,0,0.18)", animation: "fadeUp 0.3s ease" }}>
      ...
      <div className="par-wrap">
        ...
        <div className="par-topbar">
          ...
          <div className="par-tb-r">
            ...
            <div className="par-icon-btn" onClick={() => setShowNotifs((v) => !v)} style={{ cursor: "pointer" }}>
            ...
            <div className="par-u-chip">
              <div className="par-u-av">AZ</div>
              <div>
        ...
        <div className="par-layout">
          ...
          <div style={{ background: "#fff", borderRadius: 18, padding: "14px", boxShadow: "0 1px 6px rgba(0,0,0,0.05)", animation: "fadeUp 0.35s ease 0.08s both" }}>
            ...
            <div style={{ marginTop: 14, paddingTop: 14, borderTop: `1px solid ${C.border}` }}>
              <div className="par-sidebar-item" onClick={() => navigate("/login")} style={{ color: "#991b1b" }}>
          ...
          <div>
            ...
            {activeSection !== "apropos" && (
              <div style={{ display: "flex", justifyContent: "flex-end", gap: 10 }}>
                ...
                <button onClick={handleSave} style={{ padding: "10px 28px", borderRadius: 10, border: "none", background: C.primary, color: "#fff", fontWeight: 700, fontSize: 14, cursor: "pointer", boxShadow: "0 4px 14px rgba(45,96,79,0.3)" }}>
