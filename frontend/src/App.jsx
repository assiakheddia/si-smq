...
function AppLayout({ children, active }) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#eaf5eb" }}>
      ...
      <Sidebar collapsed={collapsed} setCollapsed={setCollapsed} sidebarWidth={sidebarWidth} activeNav={active} />
  ...
  return (
    <BrowserRouter>
      <Routes>
        ...
        <Route path="/dashboard" element={<AppLayout active="dashboard"><Dashboard /></AppLayout>} /> {/* ADD THIS ROUTE */}
        <Route path="*" element={<Navigate to="/login" replace />} />
      </Routes>
    </BrowserRouter>
  );
